...
export class UIManager {
    ...
    private setupStatusBar() {
        try {
            ...
            if (this.plugin.app.workspace && "statusBar" in this.plugin.app.workspace) {
                // @ts-ignore - Handle potential missing statusBar API
                this.statusBarItem = this.plugin.app.workspace.statusBar?.addStatusBarItem();
            }
        } catch (e) {
    ...
    private addAnalysisRibbonIcon() {
        const ribbonIcon = this.plugin.addRibbonIcon('brain-cog', 'Analyze Daily Journal', async () => {
            try {
                await this.plugin.serviceManager.journalAnalysisService.analyzeDailyJournal();
            } catch (error) {
    ...
    private addAnalysisRibbonIcon2() {
        const ribbonIconEl = this.plugin.addRibbonIcon(
            ...
            'Analyze Daily Journal',
            async () => {
                const logger = this.plugin.serviceManager.logger;
                ...
                try {
                    ...
                    if (!dailyNote) {
                        new Notice("No journal entry found for today");
                            logger.warn(`No journal entry found for today (${formattedDate})`);
                        return;
                    }
