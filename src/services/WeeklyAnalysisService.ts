...
export class WeeklyAnalysisService {
	...
	// Explicitly declare aiService as non-optional
	...
	constructor(
		...
		if (!aiService) {
			throw new Error(
				"AI Service must be provided to WeeklyAnalysisService"
			);
		}
		// Explicitly assign aiService to the class property
	...
	async runWeeklyAnalysis(): Promise<void> {
		...
		this.logger?.info(
			`Found ${entries.length} journal entries for analysis`
		);
	...
	async getPastWeekEntries(): Promise<{ date: string; content: string }[]> {
		...
		this.logger?.debug(
			`Looking for entries after ${new Date(oneWeekAgo).toISOString()}`
		);
	...
	async analyzeWeeklyContent(
		...
		try {
			...
			this.logger?.error(
				"Error during weekly content analysis",
				error as Error
			);
			throw error;
		}
	...
	async createWeeklyReflectionNote(analysis: string): Promise<void> {
		...
		try {
			...
			this.logger?.error(
				"Error creating weekly reflection note",
				error as Error
			);
			throw error;
		}
